...
(function() {
  $(document).on('turbolinks:load', function() {
    var a, beep, fast_pause, has_sound_on_terminate, hours, long_pause, minutes, seconds, state, updateView, work_cycle;
    seconds = 0;
    minutes = 25;
    hours = 0;
    state = 0;
    ...
    updateView = function() {
      var stopwatch_hour, stopwatch_minute, stopwatch_second, timer;
      if (hours < 10) {
    ...
    window.getPomodoroSettings = function() {
      ...
      return has_sound_on_terminate = $("input#has_sound_on_terminate").prop('checked');
    };
    window.stopwatch_start = function() {
      if (state !== 1) {
        state = 1;
        updateView();
        return window.interval = window.setInterval(function() {
    ...
    beep = function(vol, freq, duration) {
      ...
      return v.stop(a.currentTime + duration * 0.001);
    };
    window.stopwatch_pause = function() {
      state = 2;
      return clearInterval(window.interval);
    };
    window.stopwatch_stop = function() {
      state = 0;
    ...
    $('#btnFastPause').on('click', function() {
      if (state === 0) {
        window.getPomodoroSettings();
        if (fast_pause > 0) {
          minutes = fast_pause;
    ...
    return $('#btnLongPause').on('click', function() {
      if (state === 0) {
        window.getPomodoroSettings();
        if (long_pause > 0) {
          minutes = long_pause;
...
(function() {
  $(document).on('turbolinks:load', function() {
    $('.edit-settings').on('click', (function(_this) {
      return function(e) {
    ...
    return $('.change-settings').on('submit', function(e) {
      var id, requestType, requestUrl;
      id = $("#stopwatch_id").val();
      if (id === '') {
        requestType = 'POST';
      ...
      $.ajax(requestUrl, {
        ...
        success: function(data, text, jqXHR) {
          ...
          M.toast({
            html: 'Settings saved!&nbsp;<b>:)</b>',
            inDuration: 4000,
            classes: 'green'
          });
          return window.getPomodoroSettings();
        },
        error: function(jqXHR, textStatus, errorThrown) {
          if (jqXHR.status === 422) {
            ...
            return M.toast({
              html: 'Problem in Save Settings &nbsp;<b>:(</b>',
              inDuration: 4000,
              classes: 'red'
            });
          }
